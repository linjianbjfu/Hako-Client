#!/usr/bin/env python3
"""Encode application video or ordered screenshots into a verified GIF."""
...
def video_duration(ffprobe: str, path: Path) -> float:
    """Read container duration because some WebM files omit stream duration."""
    result = run_json([
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="hako-record-demo-") as temporary:
        if is_video:
            inputs = ["-i", str(source)]
